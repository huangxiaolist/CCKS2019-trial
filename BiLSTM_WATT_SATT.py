...
class PersonModel(object):
	...
	def load_data(self):
		...
		self.logger.info('Document count [{}]: {}, [{}]: {}'.format('train', len(self.data['train']), 'dev', len(self.data['dev'])))
	...
	def GCNLayer(self, gcn_in, in_dim, gcn_dim, batch_size, max_nodes, max_labels, adj_ind, adj_data, w_gating=True, num_layers=1, name="GCN"):
		...
		out = []
		out.append(gcn_in)
		# We will iterate how much times
		for layer in range(num_layers):
			...
			with tf.name_scope('%s-%d' % (name,layer)):
				...
				# We will calc the relations in every edge
				for lbl in range(max_labels):
					...
					with tf.name_scope('in_arcs-%s_name-%s_layer-%d' % (lbl, name, layer)):
						# I think the 0 is specify column, 1 is row
						inp_in  = tf.tensordot(gcn_in, w_in, axes=[2,0]) + tf.expand_dims(b_in, axis=0)
						# This function is critical for GCN
						def map_func1(i):
							# This will make a matrix and if there is a edge between src and dest , then the value is 1
							# Explaination:
							# adj_ind[lbl, i] = [(src1, dest1), (src2, dest2), (src3, dest3), .....], they point out the index
							# adj_data[lbl, i] = [value1, value2, value3, .....], they point out the value of above index
	...
	def add_model(self):
		...
		with tf.variable_scope('Bi-LSTM') as scope:
			...
			# val[0] == fw_outputs [batch_size, max_time, embed_dim]
	...
	def predict(self, sess, data, wLabels=True, shuffle=False, label='Evaluating on Test'):
		...
		for step, batch in enumerate(self.getBatches(data, shuffle)):
			...
			if step % 100 == 0:
				self.logger.info('{} ({}/{}):\t{:.5}\t{:.5}\t{}'.format(label, bag_cnt, len(self.data['dev']), np.mean(accuracies)*100, np.mean(losses), self.p.name))
		...
		if not self.p.only_eval:
			...
			for epoch in range(self.p.max_epochs):
				...
				if epoch % 2 == 0:
					...
					''' Evaluation on Dev '''
					saver.restore(sess, save_path)
					test_loss, test_acc, y, y_pred, logit_list, y_hot = self.predict(sess, self.data['dev'])
					...
					# P@All
					all_100, all_200, all_300 = self.getPscore(self.data['dev'], label='P@All Evaluation')
